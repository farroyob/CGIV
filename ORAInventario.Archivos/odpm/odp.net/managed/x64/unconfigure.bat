...
REM did the install create TNS_ADMIN reg value? If so, delete the key
reg query HKEY_LOCAL_MACHINE\SOFTWARE\Oracle\ODP.NET.Managed\4.122.21.1\ /v TNS_ADMIN /reg:64 1>NUL 2>NUL
if %ERRORLEVEL% EQU 0 (

REM remove registry entries for ODPM
echo Windows Registry Editor Version 5.00                     >  "%crt_dir%\odpm.reg"
echo [-HKEY_LOCAL_MACHINE\SOFTWARE\Oracle\ODP.NET.Managed\4.122.21.1]  >> "%crt_dir%\odpm.reg"
	
REM regedit /s "%crt_dir%\odpm.reg"
reg import "%crt_dir%\odpm.reg" /reg:64 1>NUL 2>NUL
